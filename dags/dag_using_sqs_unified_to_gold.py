...
SQS_QUEUE_URL = (
    "https://sqs.ap-northeast-2.amazonaws.com/790813558211/s3-parquet-trigger-queue"
)
...
def fetch_from_s3(event: dict | None = None) -> dict:
    ...
    if LAST_MODIFIED_PATH.exists():
        if LAST_MODIFIED_PATH.read_text().strip() == latest_modified:
            logger.info(
                "silver 데이터 변경 없음 (LastModified=%s) — 처리 스킵", latest_modified
            )
            return {
                "skip": True,
                "parquet_keys": [],
    ...
    logger.info(
        "silver 데이터 변경 감지 (LastModified=%s, 파일 수=%d)",
        latest_modified,
        len(keys),
    )
...
def extract_entities(**ctx) -> None:
    ...
    def _update_domain(domain, ts, sid):
        ...
        b = domain_bucket.setdefault(
            domain, {"first_seen": ts, "last_seen": ts, "sessions": set()}
        )
        if ts:
            if not b["first_seen"] or ts < b["first_seen"]:
                b["first_seen"] = ts
        ...
        for ev in raw_sess.get("timeline", []):
            ...
            if source == "zeek_dns":
                ...
                _add(
                    ...
                    "TRIGGERED",
                    "alert",
                    str(ev.get("signature_id") or ev.get("signature")),
                    sid,
                )
